...
public class GreaterThanOp implements Groundable {
    @Override
    public Atom execute(List<Atom> args) {
        if (args == null || args.size() != 2) {
            throw new IllegalArgumentException("GreaterThanOp requires exactly two arguments.");
        }
        try {
            ...
            throw new IllegalArgumentException("Arguments to GreaterThanOp must be numbers parsable from SymbolAtom names.", e);
        } catch (ClassCastException e) {
            throw new IllegalArgumentException("Arguments to GreaterThanOp must be SymbolAtoms representing numbers.", e);
        }
    }
}
